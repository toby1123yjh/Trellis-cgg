...
def get_current_task(
    ...
    *,
    ...
    try:
        ...
        # Projects initialized by an older Trellis runtime do not yet expose
        # the optional resolver controls. Preserve hook operation while the
        # project is updated; the default resolver remains session-scoped.
        active = resolve_active_task(
            Path(repo_root), input_data, platform=resolved_platform
        )
    if require_existing and getattr(active, "stale", False):
        return None
...
def truncate_utf8(data: bytes, cap: int) -> bytes:
    ...
    ``cap <= 0`` means "no limit" 鈥?returns ``data`` unchanged.
    """
...
def _truncate_notice(path: str, cap: int) -> str:
    return f"\n[Trellis: truncated at {cap} bytes 鈥?read {path} for the full content]"
...
def _binary_notice(path: str, size: int, reason: str) -> str:
    return (
        f"[Trellis: not inlined (binary file) 鈥?"
...
def _index_notice(path: str, size: int, reason: str) -> str:
    return (
        f"[Trellis: not inlined (total context limit reached) 鈥?"
        f"{path} ({size} bytes): {reason}]"
    )
...
def read_jsonl_entries(base_path: str, jsonl_path: str) -> list[dict]:
    ...
    Schema:
        ...
        {"_example": "..."}          # seed row 鈥?skipped (no `file` field)
    ...
    if not os.path.exists(full_path):
        print(
            f"[inject-subagent-context] WARN: {jsonl_path} not found 鈥?"
        ...
        with open(full_path, "r", encoding="utf-8") as f:
            for line in f:
                ...
                try:
                    ...
                    if not file_path:
                        # Seed / comment row 鈥?skip silently
                        continue
    ...
    if not saw_real_entry:
        print(
            ...
            f"entries (only seed / empty) 鈥?sub-agent will receive only "
...
3. **Spec sync** - Analyze whether changes introduce new patterns, contracts, or conventions
   - If new pattern/convention found: read target spec file 鈫?update it 鈫?update index.md if needed
...
def get_research_context(repo_root: str, task_dir: str | None) -> str:
    """
    Context for Research Agent 鈥?project structure overview for spec directories.
    ...
    if spec_root.is_dir():
        ...
        for i, pkg_dir in enumerate(pkg_dirs):
            ...
            prefix = "鈹斺攢鈹€ " if is_last else "鈹溾攢鈹€ "
...
def _handle_codex_subagent_start(input_data: dict) -> None:
    ...
    # Payload cwd first, then our own 鈥?some hosts (CodeBuddy IDE 4.10.4)
...
def main():
    ...
    if subagent_type in AGENTS_REQUIRE_TASK:
        ...
        # does not rewrite session files 鈥?so a poisoned pointer outlives the
