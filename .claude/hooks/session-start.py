...
3. If neither provides a language, output the language-neutral fallback exactly: `Trellis SessionStart 鉁揱.
...
def _build_first_reply_notice(update_hint: str | None) -> str:
    ...
    The reminder has to reach the *user*, not just the model's context 鈥?a line
...
# cp936 / cp1252 / etc. 鈥?non-ASCII content (Chinese task names, prd snippets)
...
def _has_curated_jsonl_entry(jsonl_path: Path) -> bool:
    ...
    ``file`` key) 鈥?that is NOT "ready". Readiness requires at least one
...
def _detect_platform(input_data: dict) -> str | None:
    ...
    # alongside their own variable 鈥?CodeBuddy, ZCode and Trae all do. It must
    ...
    # Observed on CodeBuddy IDE 4.10.4: session file `codebuddy_ae54840e鈥?json`
    # alongside marker `update-check-claude_ae54840e鈥?marker`, same id.
    env_map = {
...
def _persist_context_key_for_bash(context_key: str | None) -> None:
    ...
    without bound 鈥?one line per SessionStart forever. Skip the write when the
...
def _last_context_key_export(env_file: str) -> str | None:
    ...
    otherwise raise UnicodeDecodeError, which is a ValueError 鈥?not an OSError 鈥?
...
def _resolve_update_hint(trellis_dir: Path, context_key: str | None) -> str | None:
    ...
    try:
        ...
        # Older initialized projects expose the same probe privately and do
        # not yet scope its marker by context. Keep the reminder functional
        # while remaining compatible with those project runtimes.
        from common.session_context import _get_update_hint as get_update_hint  # type: ignore[import-not-found]

    try:
        return get_update_hint(trellis_dir.parent, context_key)
    except TypeError:
        return get_update_hint(trellis_dir.parent)
    except Exception:
...
def _extract_range(content: str, start_header: str, end_header: str) -> str:
    ...
    End header missing 鈫?extracts to end of file.
    """
...
Context loaded. Follow <task-status>. Load workflow/spec/task details only when needed.
    ...
    # conversation context 鈥?no JSON envelope. Emit the bare overview text.
